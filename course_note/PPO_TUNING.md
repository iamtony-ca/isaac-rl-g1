...
# ═══ 심화편 (Part II) — "완벽 가이드"용 ═══

> Part I이 **손잡이별 설명**이면, Part II는 그 위의 층: **① 손잡이들이 서로 어떻게 얽히나 ② 이론이 왜 이 튜닝을 강제하나 ③ 실패 유형별 근본원인 진단 ④ 로봇 형태가 튜닝을 바꾸는 이유 ⑤ 제대로 된 실험 방법론.**
> 이론 바닥은 전부 [[PPO_PAPER_REVIEW]](논문↔코드 대조). 값을 외우지 말고 **축·원리로 생각**하는 게 이 편의 목표.

---

## 8. 하이퍼파라미터는 독립이 아니다 — 커플링 지도

가장 흔한 튜닝 실패: 한 손잡이를 "그 절 설명만 보고" 돌리는 것. 실제론 여러 손잡이가 **같은 물리량을 공유**한다. 12개 손잡이는 사실 **3개의 축**으로 묶인다.

### 축 A — "한 iteration에 정책을 얼마나 움직이나" (스텝 크기)
공유 손잡이: `desired_kl` · `num_learning_epochs` · `num_mini_batches` · 배치크기 · `clip_param`
- 전부 "수집한 데이터로 정책을 얼마나 밀어붙이나"를 조절 → **하나를 키우면 다른 걸 줄여 균형**을 맞춰야.
- 예: `num_learning_epochs` 5→10 = 같은 데이터 2배 우려먹음 → 정책이 수집정책에서 더 멀어짐 → KL↑ → **adaptive가 자동으로 lr을 깎아 상쇄**. (그래서 rsl_rl에선 epochs를 올려도 잘 안 터짐 — KL 온도조절기가 받아줌)
- ⚠️ 만약 `schedule="fixed"`였다면 이 자동상쇄가 없어 epochs↑가 즉시 붕괴로 감.
- [desired_kl↑, epochs↑, batch↓, clip↑] = 전부 "더 공격적" 방향. **동시에 여러 개 공격적 = 붕괴.**

### 축 B — "탐험을 얼마나 유지하나" (탐험)
공유 손잡이: `init_noise_std`(출발 σ) · `entropy_coef`(σ 유지력) · `desired_kl`(σ가 변하는 속도 상한)
- init_noise_std = 시작 탐험량, entropy_coef = 그걸 유지하는 힘, desired_kl = 한 스텝에 σ가 얼마나 빨리 줄/늘 수 있나. **셋이 같은 σ 궤적을 만든다.**
- entropy_coef=0인데 init_noise_std만 키워봐야 금방 수렴(유지력 0). entropy_coef만 키우고 desired_kl이 작으면 σ가 크게 못 움직임.
- 두 부록 실험이 각각 이 축의 손잡이 하나(entropy_coef, desired_kl)만 건드린 것.

### 축 C — "미래 시야 ↔ 보상 크기 ↔ 에피소드 길이"
공유: `gamma` · reward weight 절대크기 · `episode_length_s`
- gamma↑ → value가 더 많은 미래 항의 합 → **value 크기 자체가 커짐** → value_loss 스케일 변함(원리 3 주의).
- 유효 시야 1/(1−γ)가 에피소드 길이보다 길면 못 보는 미래 = 의미 없음.

> **커플링 원칙: 한 축에서 한 손잡이만 움직이고 나머지는 고정.** 축을 넘나들며 여러 개 동시에 바꾸면 원인 추적 불가.

---

## 9. 이론이 튜닝을 결정하는 5가지 원리

각 원리 = [[PPO_PAPER_REVIEW]]의 개념 → "그래서 이렇게 튜닝".

**원리 1. On-policy의 근본 긴장 — 샘플 효율 vs 안정성.**
PPO 데이터는 **현재 정책 근처에서만 유효**(surrogate 근사, [[PPO_PAPER_REVIEW]] 2·3절). `num_learning_epochs`↑ = 재사용↑(효율↑) but 정책이 수집정책에서 멀어져 근사가 깨짐(불안정). **이 긴장이 PPO 튜닝의 절반**이고 clip·adaptive KL이 그 관리 장치. → "효율 원하면 epochs↑ + desired_kl↓(스텝 무르게)로 상쇄" 조합이 나오는 이유.

**원리 2. 배치 크기와 그래디언트 분산 — 왜 배치↑가 안정인가.**
그래디언트 추정 분산 ∝ 1/배치크기. 배치 크면 방향 추정 노이즈↓ → 더 자신 있게 큰 스텝. **"지글지글"의 1차 처방이 배치↑인 건 실험칙이 아니라 통계.** 병렬 로봇 4096개의 진짜 목적이 이 분산 감소.

**원리 3. Advantage 정규화 ⇒ "보상 절대크기는 씻기고 비율만 남는다".**
rsl_rl은 advantage를 평균0·분산1로 정규화([[PPO_PAPER_REVIEW]] 6-B). 모든 reward에 ×10 해도 정규화 후 advantage는 거의 동일 → **정책 그래디언트는 보상 전역 스케일에 불변.** ⇒ **[[DAY4_reward_shaping]]의 "가중치 절대값이 아니라 항들 사이 비율이 중요"의 이론적 근거.**
- ⚠️ 단, value_loss는 정규화 안 된 returns를 회귀 → 보상 스케일에 민감(∝스케일²). value_loss_coef(1.0)를 통해 극단적 보상 크기는 여전히 학습을 흔들 수 있음. **비율은 자유롭게, 전역 스케일은 상식선 유지.**

**원리 4. Adaptive KL = 소프트 트러스트 리전.**
desired_kl은 사실상 TRPO의 신뢰영역 반경 δ([[PPO_PAPER_REVIEW]] 5절)를 lr로 강제한 것. → desired_kl은 "학습률"이 아니라 **"한 번에 허용하는 정책 변화량"**으로 읽어야 맞다. 어려운 태스크(rough·biped)일수록 나쁜 방향으로 크게 가면 회복 불가 → δ 작게(보수적). 쉬운 태스크는 크게 질러도 됨(부록 kl 실험).

**원리 5. 유효 시야 = 1/(1−γ).**
γ=0.99 → 100스텝 = 2초(50Hz). 태스크의 "인과 지평"이 이보다 길면 γ↑ 필요(단 분산↑·난이도↑). 보행은 즉각적이라 0.99로 충분. λ(0.95)는 그 100스텝 advantage를 얼마나 V에 의존해 부드럽게 추정하나(bias↔variance).

---

## 10. 시나리오별 진단 플레이북 (증상 → 근본원인 이론 → 처방)

Part I 6절의 한 줄 표를 **근본원인 이론**까지 확장. 처방은 위→아래 순서로 시도.

### 10-1. 정책 붕괴 (학습 중 reward 급락 후 회복 못 함)
- **증상**: 잘 오르던 eplen이 어느 순간 폭락, `Loss/surrogate` 튐, noise_std 급변.
- **이론**: 한 업데이트에서 정책이 신뢰영역 밖으로 점프 → advantage 부호 뒤집힌 채 큰 스텝 → 파괴적 자기강화. clip이 못 막은 케이스(배치가 특정 방향 편향).
- **처방**: desired_kl↓(0.01→0.005) → num_learning_epochs↓(5→3) → 배치↑ → (최후)clip_param↓(0.2→0.1).

### 10-2. 조기 수렴 (premature convergence, 나쁜 지역최적)
- **증상**: noise_std 빠르게 바닥, eplen 낮은 값에서 정체, reward 평평.
- **이론**: 탐험이 꺼져 초반의 그럭저럭한 동작에 갇힘. entropy 항 약해 σ 붕괴. **biped에서 흔함**(균형 못 잡고 "가만히 서기"에 수렴).
- **처방**: entropy_coef↑(0.008→0.02), init_noise_std↑(1.0→1.5). reward도 의심([[DAY4]] 종료 페널티 과하면 "안 움직이기"로 도망).

### 10-3. 영원히 안 정리됨 (수렴 실패)
- **증상**: eplen은 높은데 reward 곡선 계속 진동, noise_std 높게 유지.
- **이론**: 탐험 과잉 — entropy 항이 σ를 계속 키워 정책이 한 해로 못 모임(부록 ent=0.05, eplen 38 정체).
- **처방**: entropy_coef↓. 후반이면 σ가 자연히 줄어야 정상 — 안 줄면 과탐험.

### 10-4. Critic이 못 따라옴 (value 학습 실패)
- **증상**: `Loss/value_function` 안 줄고 큼. actor도 덩달아 불안정.
- **이론**: V(s)가 return 못 맞추면 → advantage=노이즈 → 정책 그래디언트 방향 엉터리([[PPO_PAPER_REVIEW]] 4절 "조용한 엔진"). obs 스케일 제각각이거나 태스크가 망 표현력 초과.
- **처방**: actor/critic_obs_normalization=True, 망 크기↑, (드물게)value_loss_coef.

### 10-5. 기어감 (너무 느린 학습)
- **증상**: 방향 맞지만 곡선 완만, `Loss/learning_rate` 1e-5 바닥(부록 kl=0.002).
- **이론**: desired_kl 너무 작아 adaptive가 lr을 계속 깎음 → 정책 거의 안 변함.
- **처방**: **max_iterations↑로 그냥 더 오래**(대부분 "안 됨"은 "덜 됨") → 그래도면 desired_kl↑(0.01→0.02).

### 10-6. 지글지글 노이즈 (곡선 거칢, 붕괴는 아님)
- **이론**: 배치 그래디언트 분산 큼(원리 2). **처방**: num_envs↑ 또는 num_steps_per_env↑.

---

## 11. 로봇 형태가 튜닝을 바꾼다 — G1(biped) vs Go2(quadruped) 실측

같은 rsl_rl·같은 기본 하이퍼파라미터인데 **수렴 난이도가 다르다.** 왜인지가 곧 튜닝 직관.

| | G1 (biped) | Go2 (quadruped) |
|---|---|---|
| DOF(action) | 37 | 12 |
| obs(flat) | 123 | 48 |
| flat 수렴 | ~1500 iter | **~300 iter** |
| 정적 안정성 | 낮음(넘어짐) | 높음(4다리 지지) |
| entropy 과잉 반응 | 완만 | **극적**(ent=0.05→eplen 13 붕괴) |

**이론적 해석**:
- **action 차원↓(12 vs 37) = 최적화 지형 단순** → Go2가 훨씬 빨리 수렴. G1은 탐색공간 넓어 더 오래·더 탐험 유지 필요 → **G1에서 entropy_coef·max_iterations가 더 중요.**
- **정적 안정성**: 4족은 가만히도 안 넘어짐 → "서 있기" 기저에서 걷기로 개선. biped는 매 스텝 능동 균형 → 나쁜 방향 한 번에 넘어짐 → **G1이 desired_kl 붕괴·조기수렴에 더 취약.**
- **Go2 entropy 극적 붕괴(가설)**: 저차원 정책이라 σ 과잉이 행동의 큰 비율을 흔듦 + 이미 쉬운 basin에 도달해 있어 "안 정리됨"이 즉시 eplen으로 드러남.

**튜닝 지침**:
- 새 로봇이 **저DOF·정적안정(4족류)** → 기본값 공격적으로 써도 됨, 실험 짧게(300iter).
- **고DOF·biped** → max_iterations 넉넉히, entropy 유지 신경, desired_kl 보수적 시작.

(근거: G1 `logs/exp_*`, Go2 `logs/go2_exp_*`, [[go2/GO2_EXPERIMENTS]] · 로봇 구조 대조 [[go2/GO2_VS_G1]])

---

## 12. 제대로 된 튜닝 방법론 (완벽 가이드의 핵심)

손잡이 지식보다 중요한 건 **실험을 어떻게 돌리나**. 여기가 초보와 숙련의 차이.

1. **베이스라인 먼저**: 기본값으로 끝까지 한 번 돌려 곡선을 안다. 비교 기준 없는 튜닝은 미신.
2. **한 번에 한 손잡이**(+원리 8: 같은 축에서 하나만). 두 개 바꾸면 어느 게 효과인지 영영 모름.
3. **eplen을 봐라, reward 숫자 말고**([[DAY3]] 교훈, 원리 3). reward는 가중치 바꾸면 스케일이 변해 비교 불가. **eplen·성공률이 진짜 신호.**
4. **짧게 스캔 → 길게 확정**: 300iter로 값 3~4개 스캔 → 이긴 값으로 풀 학습(Go2 실험 방식).
5. **seed 분산 인지**: RL은 seed에 민감. 결정적 결론엔 2~3 seed. 애매한 차이는 노이즈일 수 있음.
6. **hydra로 코드 안 고치고**(Part I 실험명령) → 재현성·기록. 로그 파일명에 조건 박기(`day4_B_heavy_actionrate.log`).
7. **바꾸기 전에 가설을 적어라**: "entropy_coef↑ → noise_std 덜 떨어져 → eplen↑ 기대". 결과가 가설과 다르면 **그게 진짜 배움.**

> **완벽 가이드 한 줄**: 손잡이는 12개지만 **축은 3개**(스텝크기·탐험·시야), **이론은 5원리**, 그리고 **방법론**(한 번에 하나·eplen·베이스라인)이 나머지를 지배한다. 값을 외우지 말고 **축과 원리로** 생각하라.
